sql = {
    ...
    `,
    insert:`CREATE PROCEDURE genera_insertar 
    (
        @prefijo		VARCHAR (30), 
    ...
    BEGIN
        ...
        PRINT @sql
    END`,
    delete:` create procedure genera_delete(
        @prefijo		VARCHAR (30), 
        @nombre_tabla	VARCHAR (50),
        @esquema_t		VARCHAR	(30),
        @esquema_p		VARCHAR (30)
    ...
    begin
        IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=@esquema_t and TABLE_NAME=lower(@nombre_tabla))--pregunta si existe la tabla de la operacion en el esquema
            begin
                ...
                SET @sql='CREATE PROCEDURE '+ @esquema_p+'.'+@prefijo+'_Delete_'+@nombre_tabla+'(@pk '+@pk_type+') as BEGIN DELETE FROM '+ @esquema_t+'.'+@nombre_tabla+' where '+@pk_name+'=@pk; END;'
                select query = @sql
                print @sql
            end
        else
    ...
    select:` create procedure genera_select (
        @prefijo		VARCHAR (30), 
        @nombre_tabla	VARCHAR (50),
        @esquema_t		VARCHAR	(30),
        @esquema_p		VARCHAR (30)
                ...
                BEGIN
                    ...
                    ELSE
                        BEGIN
                            SET @parametros=@parametros+'@'+@columna+' '+@tipo_datos+'('+@largo+'), '
                        END
                    set @where = @where+'((@'+@columna+' is NULL) or ('+@columna+'=@'+@columna+')) and '
                    FETCH NEXT FROM c_columnas
                    INTO @columna , @tipo_datos, @largo
                END
                SET @parametros=SUBSTRING(@parametros,0,LEN(@parametros))+')'
                SET @where=SUBSTRING(@where,0,LEN(@where)-4)+')'
                CLOSE c_columnas
                DEALLOCATE c_columnas;
                set @sql=@sql+@parametros + ' AS BEGIN Select * from '
                set @sql=@sql+@esquema_t+'.'+@nombre_tabla+@where+'; END'
                select query = @sql
                print @sql
            end
        else
    ...
    update:`create procedure genera_update (
        @prefijo		VARCHAR (30), 
        @nombre_tabla	VARCHAR (50),
        @esquema_t		VARCHAR	(30),
        @esquema_p		VARCHAR (30)
    ...
    begin
        IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=@esquema_t and TABLE_NAME=lower(@nombre_tabla))--pregunta si existe la tabla de la operacion en el esquema
            begin
                ...
                END
                SET @parametros=SUBSTRING(@parametros,0,LEN(@parametros))+')'
                SET @update=SUBSTRING(@update,0,LEN(@update))+' where '+@pk_name+'=@pk_'+@pk_name
                CLOSE c_columnas
                DEALLOCATE c_columnas;
                set @sql=@sql+@parametros + ' AS BEGIN Update '
                set @sql=@sql+@esquema_t+'.'+@nombre_tabla+@update+'; END'
                select query = @sql
                print @sql
            end
        else
